<template>
	<section>
		...
		<el-col :span="24" v-show="details">
				...
				<el-col :span="18" :offset="3" class="body_text">
					<el-col :span="19" :offset="2" style="margin-top: 10px">
						<el-form :model="orderDetails" label-width="80px" :rules="editFormRules" :inline="true" ref="editForm">
							<el-form-item :label="item.key" v-for="item in paramsData" style="margin-right:  200px;">
			...
			<el-button type="primary" @click="release" style="margin: 20px;">修改发布</el-button>
		...
		methods: {
            ...
            getPList(){
            	...
            	$.ajax({
                    ...
                    success:function(data){
                    	...
                    	console.log(_this.biaoqanList)
                    	console.log(info.list)
                    	for (var i = 0; i < _this.biaoqanList.length; i++) {
                    		for (var x = 0; x < info.list.length; x++) {
                    			if(_this.biaoqanList[i] === info.list[x].id.toString()){
                    				_this.checkedCities.push(info.list[x].name)
                    			}
                    		}
                    	}
                    }
            ...
            getlist() {
				...
				$.ajax({
                    ...
                    success:function(data){
                    	...
                    	const info = data.data
                    	// 分类id
                    	_this.fenleiId = info.catId
                    	
                    	// 商品品牌
                    	_this.paramsData = eval('(' + info.goodsData + ')')

                    	// 商品名称
                    	_this.CommodityName = info.name
                    	// 商品描述
                    	_this.DescriptionGoods = info.goodsDesc
                    	// 商品货号
                    	_this.productCode = info.goodsNo
                    	// 商品图片
                    	_this.CommodityPictures = info.carouselPicture.split(',')
                    	// 计量单位
                    	_this.MeasurementUnit = info.unit
                    	// 建议零售价
                    	...
                    	// value
                    	_this.value = info.ftId
                    	//定价模式
                    	_this.PricingModel = info.pricingModel.toString()

                    	_this.NextStep()
                    	_this.biaoqanList = info.goodsServiceStr.split(',')
                    	// const info = data.data
            ...
            selectListName(){
                ...
                $.ajax({
                    ...
                    success:function(data){
                    	console.log(data)
                    	console.log(_this.fenleiId)
                    	for (var i = 0; i < data.data.length; i++) {
                    		if(data.data[i].id === _this.fenleiId){
                    			_this.commodity = data.data[i].name
                    		}
                    	}
                    }
                })
